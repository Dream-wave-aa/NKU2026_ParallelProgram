...
void PriorityQueue::Generate(PT pt)
{
    ...
    if (pt.content.size() == 1)
    {
        ...
        if (total_values < 1000) {
            ...
            // 获取线程数（可以在外部通过 omp_set_num_threads 设置）
            int num_threads = omp_get_max_threads();
            
            // 每个线程的私有结果
            vector<vector<string>> thread_results(num_threads);
            ...
            #pragma omp parallel
            {
                ...
                thread_results[thread_id] = move(local_vec);
                thread_counts[thread_id] = local_cnt;
            }
            
            // 主线程合并所有结果
            for (int t = 0; t < num_threads; t++) {
                for (const string& guess : thread_results[t]) {
                    guesses.emplace_back(guess);
                }
                total_guesses += thread_counts[t];
            }
        }
    }
    else
    {
        ...
        if (total_values < 1000) {
            ...
            #pragma omp parallel
            {
                ...
                #pragma omp for schedule(static)
                for (int i = 0; i < total_values; i++) {
                    local_vec.emplace_back(prefix + a->ordered_values[i]);
                    local_cnt++;
                }
