...
int main()
{
    ...
    // // 设置 OpenMP 线程数
    // #ifdef _OPENMP
    // omp_set_num_threads(4);  // 使用 4 个线程，可以根据 CPU 核心数调整
    // //cout << "OpenMP enabled with " << omp_get_max_threads() << " threads" << endl;
    // #endif
    ...
    while (!q.priority.empty())
    {
        ...
        if (curr_num > 1000000)
        {
            ...
            //     // // MD5Hash_SIMD4 版本 一次性生成4个MD5Hash
            //     //     int n = q.guesses.size();
            //     //     int processed = 0;
            //     //     while (processed + 4 <= n) {
            //     //         vector<string> batch;
            //     //         for (int k = 0; k < 4; k++) {
            //     //             batch.push_back(q.guesses[processed + k]);
            //     //         }
            //     //         bit32 states[4][4];
            //     //         MD5HashSIMD(batch, states);
            //     //         processed += 4;
            //     //     }
            //     //     // 处理剩余不足4个的口令
            //     //     for (; processed < n; processed++) {
            //     //         bit32 state[4];
            //     //         MD5Hash(q.guesses[processed], state);
            //     //     }
            ...
            // }
             ...
             // SIMD版本：每次处理4个密码
           // 优化的SIMD批处理：处理完整批次
            size_t guess_size = q.guesses.size();
            size_t i = 0;
            
            // 处理完整的4倍数组 - 使用SIMD
            for (; i + 3 < guess_size; i += 4)
            {
                string batch[4];
                batch[0] = q.guesses[i];
                batch[1] = q.guesses[i + 1];
                batch[2] = q.guesses[i + 2];
                batch[3] = q.guesses[i + 3];
                
                bit32 states[4][4];
                MD5Hash_SIMD_4(batch, states);
            }
